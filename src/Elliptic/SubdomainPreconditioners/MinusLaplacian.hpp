...
class MinusLaplacian
 ...
 private:
  ...
  using SolverType = tmpl::conditional_t<std::is_abstract_v<Solver>,
 ...
 public:
  ...
  MinusLaplacian(SolverType solver) noexcept : solver_(std::move(solver)) {}
  ...
  void reset() noexcept override { solver().reset(); }

  // NOLINTNEXTLINE(google-runtime-references)
  void pup(PUP::er& p) noexcept override { p | solver_; }
 ...
 private:
  SolverType clone_solver() const noexcept {
    if constexpr (std::is_abstract_v<Solver>) {
...
void assign_component(
    ::LinearSolver::Schwarz::ElementCenteredSubdomainData<Dim, LhsTagsList>&
        lhs,
...
MinusLaplacian<Dim, OptionsGroup, Solver, LinearSolverRegistrars>::solve(
  ...
  for (size_t component = 0;
       ...
       ++component) {
    detail::assign_component(source_, source, 0, component);
    detail::assign_component(initial_guess_in_solution_out_,
