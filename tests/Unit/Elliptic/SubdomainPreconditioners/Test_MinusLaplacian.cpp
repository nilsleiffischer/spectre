...
SPECTRE_TEST_CASE("Unit.Elliptic.SubdomainPreconditioners.MinusLaplacian",
                  "[Unit][Elliptic]") {
  {
    ...
    const auto created = TestHelpers::test_creation<
        std::unique_ptr<::LinearSolver::Serial::LinearSolver<tmpl::list<
  ...
  }
  {
    ...
    constexpr size_t Dim = 2;
    const auto created = TestHelpers::test_creation<
        std::unique_ptr<::LinearSolver::Serial::LinearSolver<
    ...
    REQUIRE(dynamic_cast<const MinusLaplacian<Dim, OptionsGroup>*>(
                created.get()) != nullptr);
    const auto& minus_laplacian =
        dynamic_cast<const MinusLaplacian<Dim, OptionsGroup>&>(*created);
    const auto& solver = minus_laplacian.solver();
    REQUIRE(
        dynamic_cast<
