...
void test_restrict_fields(const Mesh<1>& fine_mesh, const Mesh<1>& coarse_mesh,
                          DataVector fine_data_left, DataVector fine_data_right,
  ...
  const auto add_element =
      [&runner](const ElementId<Dim>& element_id,
                ...
                std::unordered_set<ElementId<Dim>> child_ids, Mesh<Dim> mesh,
                std::optional<Mesh<Dim>> parent_mesh,
                std::optional<DataVector> data) {
        ...
        ActionTesting::emplace_component_and_initialize<element_array>(
            ...
            {std::move(parent_id), std::move(child_ids), std::move(mesh),
             std::move(parent_mesh), size_t{0}, std::move(fields)});
      };
  ...
  }
  {
    DataVector coarse_data =
        ...
        apply_matrices(
            ...
            fine_data_right, Index<1>{4});
    test_restrict_fields<MassiveTag>(fine_mesh, coarse_mesh, fine_data_left,
                                     fine_data_right, std::move(coarse_data),
                                     true);
  }
}
